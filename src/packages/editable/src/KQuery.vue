<template>
  <div class="field-query">
    <div class="field-query-block flex_row" layout-align="space-between stretch">
      <input
        ref="contentInput"
        type="text"
        flex
        @focus="openPanel($event.target.value)"
        :placeholder="value"
        @input="search($event.target.value)"
      />
      <span class="label-icon flex_row" @click.stop="show = true" layout-align="center center">
    ...
    >
      <table class="sl-bx-table k_e_t">
        <thead>
          <tr>
            <td v-for="(cell, i) in fields" :key="i">{{ cell.label }}</td>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(item, index) in dataList" @click="choose(item)" :key="index">
            <td v-for="(cell, i) in fields" :key="i">{{ item[cell.name] }}</td>
          </tr>
        </tbody>
      </table>
    </div>
